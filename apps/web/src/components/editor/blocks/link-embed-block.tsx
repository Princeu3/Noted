...
export const createLinkEmbedBlock = createReactBlockSpec(
  {
  ...
  },
  {
    render: ({ block, editor }) => {
      ...
      useEffect(() => {
        ...
        setLoading(true);
        api<OGData>(
          `/api/embeds/opengraph?url=${encodeURIComponent(block.props.url)}`,
        )
          .then(setData)
          .catch(() => {})
      ...
      const title = data?.title || block.props.url;
